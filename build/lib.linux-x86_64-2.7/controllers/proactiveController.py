...
class HederaController(object):
    ...
    def _install_reactive_path(self):
        ...
        data = shelve.open('reactiveFlows.db')
        for k, v in data.iteritems():
            match = of.ofp_match()
            match.dl_src = EthAddr(v['match']['dl_src'])
            match.dl_dst = EthAddr(v['match']['dl_dst'])
            match.dl_vlan = v['match']['dl_vlan']
            match.dl_vlan_pcp = 0
            match.dl_type = v['match']['dl_type']
            match.nw_tos = 0
            match.nw_proto = 6
            match.nw_src = IPAddr(v['match']['nw_src'])
            match.nw_dst = IPAddr(v['match']['nw_dst'])
            match.tp_src = v['match']['tp_src']
            match.tp_dst = v['match']['tp_dst']
            self.switches[v['dpid']].install(v['out_port'],
